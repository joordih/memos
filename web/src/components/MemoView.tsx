...
interface HeadingNode {
  level: number;
  children: { type: "TEXT"; textNode: TextNode }[];
}

interface Node {
  type: "HEADING" | "LINE_BREAK" | "PARAGRAPH";
...
const MemoView: React.FC<Props> = (props: Props) => {
  ...
  const { title, description } = getHeadings(memo);

  function getHeadings(data: Memo): { title: string | null; description: string | null } {
    let title: string | null = null;
    let description: string | null = null;
    let headingCount = 0;

    for (const node of data.nodes) {
      if (node.type === "HEADING" && node.headingNode?.children.length) {
        const text = node.headingNode.children[0].textNode.content;
        headingCount++;
